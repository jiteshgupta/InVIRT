		...
		if(foundIndex == false) 
		{
            ...
            if (usePostionTracking)
                if (originOverride != null)
                {
					...
					temp.x = initialX + (OptitrackRigidBodyManager.instance.rigidBodyPositions[index].x)*210.0f;
					temp.y = initialY - (OptitrackRigidBodyManager.instance.rigidBodyPositions[index].y)*160.0f;
					temp.z = initialZ - (OptitrackRigidBodyManager.instance.rigidBodyPositions[index].z)*220.0f;
