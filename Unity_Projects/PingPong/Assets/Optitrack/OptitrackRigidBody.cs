		...
		if(foundIndex == false) 
		{
				...
				if(foundIndex == false) 
				{
					for(int i = 0; i < OptitrackRigidBodyManager.instance.rigidBodyIDs.Length; i++) 
					{
						...
						if(OptitrackRigidBodyManager.instance.rigidBodyIDs[i] == ID) 
						{
							...
							previous.x = initialX;
							previous.y = initialY;
							previous.z = OptitrackRigidBodyManager.instance.rigidBodyPositions[index].z;
						}
					}
					foundIndex = true;
				}
            ...
            if (usePostionTracking)
                if (originOverride != null)
                {
					...
					temp.x = initialX;	
					temp.y = initialY;
					float zValue = OptitrackRigidBodyManager.instance.rigidBodyPositions[index].z;
					temp.z = zValue;
					//transform.position = temp;
					rb.velocity = (temp - previous) * -1200.0f;
					previous = temp;
				}
